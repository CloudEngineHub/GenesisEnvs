...
class GenesisEnvTrainer:
    def __init__(
        ...
        self.print_log_every_epochs = print_log_every_epochs
    ...
    def rollout(
        self, horizon: int = 50, deterministic: bool = False
    ) -> Tuple[States, Actions, Rewards, Dones]:
    ...
    def train_loop(self, num_epochs: int = 500, max_rollout_step: int = 50):
        ...
        for epoch in range(num_epochs):
            states, actions, rewards, dones = self.rollout(horizon=max_rollout_step)
            ...
            log_string = (
                f"Epoch [{epoch+1}/{num_epochs}] - {log_string} | "
                f"Elapsed: {elapsed_str} Remaining: {remaining_str} ({percent_complete:.2f}%)"
            )
            if (epoch + 1) % self.print_log_every_epochs == 0:
                logger.info(log_string)
        
            if (epoch + 1) % self.save_checkpoint_every_epochs == 0:
                check_point_save_path = os.path.join(self.checkpoint_save_path, f"checkpoint_{(epoch+1):04d}.pth")
                logger.info(f"save checkpoint: {check_point_save_path}")
                torch.save(
                    self.agent.actor_critic_model.state_dict(), check_point_save_path
                )

        check_point_save_path = os.path.join(self.checkpoint_save_path, "checkpoint_final.pth")
        logger.info(f"save checkpoint: {check_point_save_path}")
        torch.save(
            self.agent.actor_critic_model.state_dict(), check_point_save_path
        )
